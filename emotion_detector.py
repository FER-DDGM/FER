...
class ImprovedEmotionDetector:
    ...
    def detect_faces_opencv(self, frame):
        """Detect faces using OpenCV for preprocessing"""
    ...
    def update_face_tracking(self, detections):
        """Update face tracking with emotion history"""
    ...
    def get_smoothed_emotion(self, track_id):
        """Get smoothed emotion prediction using history"""
    ...
    def detect_emotions(self, frame, show_debug=False):
        """Enhanced emotion detection with optional face preprocessing"""
        ...
        for result in results:
            ...
            if boxes is not None:
                for box in boxes:
                    ...
                    if confidence > self.confidence_threshold and self.is_face_size_valid((x1, y1, x2, y2), frame.shape):
    ...
    def detect_emotions_with_face_preprocessing(self, frame, opencv_faces, show_debug=False):
        """Detect emotions using OpenCV face detection + YOLO classification"""
        detected_faces = []
    ...
    def draw_enhanced_detections(self, frame, detected_faces, show_debug=False):
        """Draw enhanced detection results with tracking"""
    ...
    def run_camera(self, show_stats=True, show_debug=False):
        ...
        while True:
            ...
            if key == ord('q'):
                ...
                print(f"Face detection mode: {'OpenCV+YOLO' if use_face_detection else 'YOLO Direct'}")
            elif key == ord('t'):
