...
class YOLOEmotionTrainer:
    ...
    def create_realistic_images(self, face_img, emotion_id, img_index, split):
        ...
        for var in range(num_variations):
            ...
            img[y1:y2, x1:x2] = (self.face_alpha * resized_face + (1-self.face_alpha) * img[y1:y2, x1:x2]).astype(np.uint8)
    ...
    def add_background_objects(self, img, face_bbox):
        ...
        for _ in range(random.randint(self.bg_objects_count_min, self.bg_objects_count_max)):
            ...
            if random.random() > 0.5:
                cv2.rectangle(img, (obj_x, obj_y), (obj_x + self.bg_object_size, obj_y + self.bg_object_size), color, -1)
            else:
                cv2.circle(img, (obj_x + self.bg_object_radius, obj_y + self.bg_object_radius), self.bg_object_radius, color, -1)
